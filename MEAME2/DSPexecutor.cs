...
namespace MEAME2
{
  ...
  public static class ExecutorTools {
	...
	if(value == 14){ return "(Function call: SET BLANKING)" ;}
	if(value == 15){ return "(Function call: SET BLANKING PROTECTION)" ;}
      }
  ...
  public class LiveExecutor : Executor {
	...
	log.info($"live executor writing:\0x{address:X} <- {word:X} {ExecutorTools.parseWrite(address, word)}");
  ...
  public class MockExecutor : Executor {
	...
	log.err("Mock connection destroyed");
      ...
      public void write(uint address, uint word) {
	log.info($"mock executor writing:\0x{address:X} <- {word:X} {ExecutorTools.parseWrite(address, word)}");
      }
